...
struct Config{
    ...
    int  transparency = 0;    // 0=完全隐藏,1=10%透明度(占位)
} g_cfg;
...
static void SaveCfg()
{
    ...
    WritePrivateProfileStringW(L"cfg",L"hideIcons",
        g_cfg.hideIcons?L"1":L"0",path);
    WritePrivateProfileStringW(L"cfg",L"showInSysTray",
        g_cfg.showInSysTray?L"1":L"0",path);
    WritePrivateProfileStringW(L"cfg",L"transparency",
        std::to_wstring(g_cfg.transparency).c_str(),path);
}
...
void ShowSettingsDlg(HWND hParent)
{
    ...
    DialogBoxParamW(GetModuleHandle(nullptr),
        MAKEINTRESOURCE(IDD_SETTINGS), hParent,
        [](HWND h, UINT m, WPARAM w, LPARAM l)->INT_PTR{
            ...
            case WM_INITDIALOG:{
                ...
                HWND c = GetDlgItem(h, IDC_TRANS);
                SendMessageW(c,CB_ADDSTRING,0,(LPARAM)L"完全隐藏");
                SendMessageW(c,CB_ADDSTRING,0,(LPARAM)L"透明度10%(占位)");
                SendMessageW(c,CB_SETCURSEL,g_cfg.transparency,0);
                return TRUE;
            }
            case WM_COMMAND:
                if(LOWORD(w)==IDOK){
